---
title: "Product Price Elasticity Analysis - By Brand"
header-includes: 
   \usepackage{graphicx}
   \usepackage{fancyhdr}
   \pagestyle{fancy}
...
## Abstract
- This report is to show the PED analysis on products by brand.
...
# calculating the timedifference - this is a reference column for PED. Ignore the negative sign. The lower the absolute value is the better accuracy when we look at PED
priceGrouped[, days_diff := c(0, `units<-`(diff(Total_sales_days), "days")), by = product_id]
...
priceGrouped <- dplyr::arrange(priceGrouped, product_id, price_id_New)
...
```{r Data_Reform_Top10, echo=FALSE, message=FALSE, warning=FALSE}
...
for (index in which(paste(Top10_PED$product_id, Top10_PED$price_id_New, sep = "")
                    %in% paste(Top10$product_id, Top10$price_id_New, sep = ""))){
